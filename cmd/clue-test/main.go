...
func main() {
	...
	steps := []step{
		...
		{label: "init 10/5 (expect: full)", h5pct: 10, w1pct: 5, full: true},
		{label: "update 50/20 (expect: bw)", h5pct: 50, w1pct: 20},
		{label: "update 85/20 — h5 turns red (expect: full)", h5pct: 85, w1pct: 20, full: true},
		{label: "update 90/23 — h5 red grows (expect: tri)", h5pct: 90, w1pct: 23},
		{label: "update 95/85 — w1 turns red (expect: full)", h5pct: 95, w1pct: 85, full: true},
		{label: "update 100/87 — both red grow (expect: tri)", h5pct: 100, w1pct: 87},
		...
		{label: "init 85/15 — h5 turns red (expect: full)", h5pct: 85, w1pct: 15, full: true},
		...
		{label: "update 80/80 — both turn red (expect: full)", h5pct: 80, w1pct: 80, full: true},
	...
	for i, s := range steps {
		...
		log.Printf("[%d/%d] %s", i+1, len(steps), s.label)
		...
		refreshDelay := 10 * time.Second // quick refresh (bw/tri, 3 passes)
		if s.full {
